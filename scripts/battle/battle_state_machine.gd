...
## Flow: INTRO -> PLAYER_TURN -> RESOLVE_ATTACK -> ENEMY_TURN -> CHECK_WIN -> loop

enum BattlePhase { INTRO, PLAYER_TURN, RESOLVE_ATTACK, ENEMY_TURN, CHECK_WIN, VICTORY, DEFEAT, DRAW }

signal phase_changed(phase: BattlePhase)
signal attack_resolved(attacker_side: String, damage: int, grade: String)

var current_phase: BattlePhase = BattlePhase.INTRO
...
func setup(p_player: Node2D, p_enemy: Node2D) -> void:
...
func _resolve_player_hit(grade: String, combo: int, note_type: String) -> void:
	...
	if note_type == "attack":
		var attacker: Node2D = player_army.call("get_front_unit")
		...
		if _attack_result_type == "super_counter":
			var defender: Node2D = player_army.call("get_front_unit")
			if defender:
...
func _enemy_attack() -> void:
	var attacker: Node2D = enemy_army.call("get_front_unit")
	var target: Node2D = player_army.call("get_front_unit")
	if attacker and target:
